module.exports = function () {
    ...
    function create(app, config, io) {
        ...
        function sget(setName, member, next) {
            console.log('sget(setName:', setName, ', member:', member, '), set:', keySets);
            // Does the set exists
            if (keySets[setName]) {
                // Get all members?
                if (member === '*') {
                    process.nextTick(function () {
                        console.log(Object.keys(keySets[setName]))
        ...
        function sdel(setName, member, next) {
            console.log(typeof(keySets))
            console.log('sget(setName:', setName, ', member:', member, ')');
            console.log('keyset:',keySets, !isUndefined(keySets[setName]), keySets[setName])
            var count = 0;
            // Does the set exists
            if (keySets[setName]) {
